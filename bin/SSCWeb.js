...
var argv    = require('yargs')
				...
				({
					'id': 'cluster1',
					'parameters': '',
					'start': '2002-01-02T10:00:00.000Z',
					'stop': '2002-01-02T10:02:30.000Z',
					'debug': 'false'
				})
				.argv;
...
if (argv.parameters === true || argv.parameters === '') {
	...
	try {
		var json = JSON.parse(fs.readFileSync(__dirname + '/../metadata/SSCWeb/SSCWeb-catalog.json', 'utf8'));
	} catch (error) {
	  	console.log(error);
  		process.exit(1);		
	}
...
var SPCR = argv.id;
var START_TIME = moment(argv.start).utc().format('YYYY+DDDD+HH:mm:ss');
...
function extractData(data) {
	...
	data = data.toString()
			...
			.filter(function(s){return s !== ''}); // Needed?

	STOP_TIME = moment(argv.stop).utc().format('YYYY-DDDDTHH:mm:ss');

	while (data.length > 0) {
		if (data[data.length - 1].slice(0,17) >= STOP_TIME) {
			if (debug) {
				console.log("Removing " + data[data.length - 1].slice(0,17));
			}
			data = data.slice(0,-1);
		} else {
			break;
